...
/** Trial-division primality test (modulus is always small here, m <= 16). */
 ...
 * is intended for the small moduli used here, m <= 48; the m = 1e6 unit test
...
// C(n, k) for small n, k (n < p <= 48) — an exact integer (max C(46,23) ≈ 8.2e12,
// well within Number's safe-integer range), used by Lucas' theorem.
function smallBinom(n: number, k: number): number {
  if (k < 0 || k > n) return 0;
  let r = 1;
  for (let t = 0; t < k; t++) r = (r * (n - t)) / (t + 1);
  return Math.round(r);
}
 ...
 * per-digit small binomials, and is 0 the moment any digit j_k > i_k. Lets the
...
export function lucasMod(i: number, j: number, p: number): number {
  ...
  while (a > 0 || b > 0) {
    ...
    const bi = b % p;
    if (bi > ai) return 0;
    result = (result * (smallBinom(ai, bi) % p)) % p;
    a = Math.floor(a / p);
    b = Math.floor(b / p);
  }
  return result;
